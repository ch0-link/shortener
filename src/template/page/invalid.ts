...
export function body (message: string): string {
  return $.html(
    base(
      header().add(
        card(
          $('<h1 class="text-red-500 text-xl font-bold space-x-2 border-b-2"><span>Invalid</span></h2>').prepend(
            $(icons.x.toSvg({
              'stroke-linecap': 'butt',
            ...
            <p>${message}</p>
              <div class="flex flex-col space-y-2 sm:(space(y-0 x-2) flex-row)">
                <a class="bg-white text-black border-white border-2 p-2" href="/">Home ↗</a>
                ...
                <noscript>
                  <style>
                    ...
                    button[data-action='support']::after {
                      content: '\\00a0 (✕ JS)';
                    }
                  </style>
                </noscript>
...
export function head (): string {
  return '<title>Invalid | ch0.link</title>'
}
